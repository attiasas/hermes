...
public final class HermesGameConfigParser {
  ...
  public static HermesGameConfig parse(File file) {
    if (!file.isFile()) {
      throw new GradleException("hermes.json not found: " + file.getAbsolutePath());
    }
    try (FileReader reader = new FileReader(file, StandardCharsets.UTF_8)) {
      JsonObject root = GSON.fromJson(reader, JsonObject.class);
      if (root == null) {
        throw new GradleException("hermes.json is empty: " + file.getAbsolutePath());
      }
      ...
      throw new GradleException("Invalid hermes.json at " + file.getAbsolutePath() + ": " + e.getMessage(), e);
    } catch (IOException e) {
      throw new GradleException("Failed to read hermes.json at " + file.getAbsolutePath(), e);
    }
  }
